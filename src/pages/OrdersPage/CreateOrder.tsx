...
function CreateOrder() {
  ...
  const [alertMessages, setAlertMessages] = useState<string[]>([]);  // New state to hold alert messages
  ...
  const handleSubmit = async (e: React.FormEvent, type: string) => {
    ...
    if (type === "contact") {
      ...
      if (res.data.length === 0) {
        alertMessages.push(`No member with number ${cred.contact}`);
        return setAlertMessages(alertMessages);
      }
      setMember(res.data[0]);
    } else if (type === "buy") {
      for (const e of basket) {
        const messages = await context?.createOrder(member._id, e._id, member.wallet, e.price);
        if (messages) alertMessages = [...alertMessages, ...messages]; 
      }
      resetOrder();
      setIsModalOpen(true);
        ...
        <DialogContent>
          ...
          <DialogDescription>
          {
                alertMessages.length > 0 ? (
                    <ul>
                    {Object.entries(alertMessages.reduce((acc: { [key: string]: number }, message) => {
                        acc[message] = acc[message] ? acc[message] + 1 : 1;
                        return acc;
                    }, {})).map(([message, count], index) => (
                        <li key={index}>{count}x {message}</li>
                    ))}
                    </ul>
                ) : (
